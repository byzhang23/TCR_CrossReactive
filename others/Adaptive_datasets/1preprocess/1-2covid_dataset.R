...
setwd('')
raw.dir = './data/ImmuneCODE-Review-002/'
output.dir = './data/proc/'
if(!dir.exists(output.dir)) dir.create(output.dir)

files = list.files(raw.dir,'.tsv')

## check files and meta 
meta = read.csv('./data/meta.csv')
#identical(sort(meta$sample_name),sort(gsub('.tsv','',files)))

## merge every 200 into 1 rds
start = seq(1,length(files),by=200)
end = ifelse(start+199>length(files),length(files),start+199)
...
saveRDS(res,paste0(output.dir,'combine_',k,'.rds'))
